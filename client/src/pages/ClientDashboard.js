...
export default function ClientDashboard() {
  ...
  return (
    <div style={{ fontFamily:"'Segoe UI',sans-serif", backgroundColor:"#0f172a", minHeight:"100vh" }}>
      ...
      {/* Hero */}
      <div style={{ background:"linear-gradient(135deg,#022c22 0%,#10b981 50%,#06b6d4 100%)", padding:"32px 32px", position:"relative", overflow:"hidden" }}>
      ...
      <div style={{ padding:32 }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:16, marginBottom:32 }}>
          {[
            { label:"Total Projects",    val:projects.length,  icon:"📁", color:"#10b981", bg:"#ecfdf5" },
            ...
            <div key={s.label} style={{ backgroundColor:"#1e293b", borderRadius:24, padding:"22px 24px", boxShadow:"0 10px 40px -10px rgba(0,0,0,0.08)", border:"none", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              <div>
        ...
        {/* Quick Actions */}
        <div style={{ backgroundColor:"#1e293b", borderRadius:24, padding:"20px 24px", marginBottom:28, boxShadow:"0 10px 40px -10px rgba(0,0,0,0.08)", border:"none", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
          <div>
            <div style={{ fontWeight:700, fontSize:15, color:"#f8fafc" }}>Ready to find talent?</div>
            <div style={{ fontSize:13, color:"#94a3b8", marginTop:2 }}>Post a new project and start receiving proposals</div>
          </div>
          <div style={{ display:"flex", gap:12 }}>
            <button onClick={() => navigate("/projects")}
              style={{ padding:"10px 20px", backgroundColor:"#1e293b", color:"#e2e8f0", border:"1.5px solid #e2e8f0", borderRadius:12, cursor:"pointer", fontWeight:600, fontSize:13 }}>
              View All Projects
            </button>
            <button onClick={() => navigate("/post-project")}
              style={{ padding:"10px 20px", background:"linear-gradient(135deg,#10b981,#06b6d4)", color:"white", border:"none", borderRadius:12, cursor:"pointer", fontWeight:600, fontSize:13, boxShadow:"0 2px 8px rgba(16,185,129,0.3)" }}>
              + Post New Project
            </button>
          </div>
        </div>
        ...
        {/* Empty State */}
        {projects.length === 0 && (
          <div style={{ backgroundColor:"#1e293b", borderRadius:24, padding:"60px 32px", textAlign:"center", boxShadow:"0 10px 40px -10px rgba(0,0,0,0.08)", border:"none" }}>
            <div style={{ fontSize:56, marginBottom:16 }}>📁</div>
            <h3 style={{ fontSize:18, fontWeight:700, color:"#f8fafc", marginBottom:8 }}>No projects yet</h3>
            <p style={{ color:"#94a3b8", fontSize:14, marginBottom:20 }}>Post your first project and start receiving proposals from top freelancers.</p>
            <button onClick={() => navigate("/post-project")}
              style={{ padding:"12px 28px", background:"linear-gradient(135deg,#10b981,#06b6d4)", color:"white", border:"none", borderRadius:10, cursor:"pointer", fontWeight:600, fontSize:14, boxShadow:"0 4px 12px rgba(16,185,129,0.3)" }}>
...
function ProjectCard({ p, proposals, navigate, recommendations, setRecommendations, recommendLoading, setRecommendLoading }) {
  ...
  const statusStyle = {
    "open":        { bg:"linear-gradient(135deg,#10b981,#06b6d4)", color:"white" },
  ...
  return (
    <div style={{ backgroundColor:"#1e293b", borderRadius:24, padding:24, boxShadow:"0 10px 40px -10px rgba(0,0,0,0.08)", border:"none", display:"flex", flexDirection:"column", gap:12 }}
      onMouseEnter={e => e.currentTarget.style.boxShadow="0 20px 40px -10px rgba(0,0,0,0.12)"}
      onMouseLeave={e => e.currentTarget.style.boxShadow="0 10px 40px -10px rgba(0,0,0,0.08)"}>

      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
        <h3 style={{ fontSize:16, fontWeight:700, color:"#f8fafc", margin:0, flex:1, marginRight:8 }}>{p.title}</h3>
      ...
      {p.skills && (
        <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
          {p.skills.split(",").slice(0,3).map(sk => (
            <span key={sk} style={{ padding:"3px 10px", background:"linear-gradient(135deg,#ecfdf5,#d1fae5)", color:"#047857", borderRadius:20, fontSize:11, fontWeight:600, border:"1px solid #a7f3d0" }}>
              {sk.trim()}
            </span>
          ))}
        </div>
      ...
      <div style={{ display:"flex", gap:16, paddingTop:12, borderTop:"1px solid #f1f5f9" }}>
        <div>
          ...
          <div style={{ fontWeight:700, color:"#f8fafc", fontSize:14 }}>${p.budget}</div>
        </div>
        <div>
          <div style={{ fontSize:11, color:"#94a3b8", textTransform:"uppercase", letterSpacing:"0.5px" }}>Deadline</div>
          <div style={{ fontWeight:700, color:"#f8fafc", fontSize:14 }}>{p.deadline || "—"}</div>
        </div>
        <div>
          <div style={{ fontSize:11, color:"#94a3b8", textTransform:"uppercase", letterSpacing:"0.5px" }}>Proposals</div>
      ...
      <button onClick={() => navigate(`/view-proposals/${p.id}`)}
        style={{ width:"100%", padding:"10px", background:"linear-gradient(135deg,#10b981,#06b6d4)", color:"white", border:"none", borderRadius:12, cursor:"pointer", fontWeight:600, fontSize:13, boxShadow:"0 2px 8px rgba(16,185,129,0.3)", marginTop:4 }}>
      ...
      <button onClick={getRecommendations}
        ...
        style={{ width:"100%", padding:"10px", background:"#1e293b", color:"#7c3aed", border:"1.5px solid #7c3aed", borderRadius:12, cursor:"pointer", fontWeight:600, fontSize:13, opacity: recommendLoading[p.id] ? 0.7 : 1 }}>
      ...
      {recommendations[p.id] && (
        <div style={{ padding:16, backgroundColor:"#faf5ff", borderRadius:12, border:"1px solid #ddd6fe" }}>
          ...
          {recommendations[p.id].map((r, i) => (
            <div key={r.id} style={{ padding:12, backgroundColor:"#1e293b", borderRadius:10, border:"1px solid #ede9fe", marginBottom:8 }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:6 }}>
                <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                  ...
                  <div>
                    <div style={{ fontWeight:700, fontSize:13, color:"#f8fafc" }}>{r.name}</div>
                    <div style={{ fontSize:11, color:"#94a3b8" }}>{r.experience || "N/A"}</div>
                  </div>
                </div>
                <div style={{ fontSize:20, fontWeight:800, color:"#7c3aed" }}>{r.score}%</div>
              </div>
              <div style={{ fontSize:12, color:"#94a3b8", marginBottom:6 }}>{r.summary}</div>
              {r.matching_skills?.length > 0 && (
                <div style={{ fontSize:11 }}>
                  <span style={{ color:"#16a34a", fontWeight:600 }}>✅ </span>
                  <span style={{ color:"#e2e8f0" }}>{r.matching_skills.join(", ")}</span>
                </div>
              )}
              {r.missing_skills?.length > 0 && (
                <div style={{ fontSize:11, marginTop:2 }}>
                  <span style={{ color:"#dc2626", fontWeight:600 }}>❌ </span>
                  <span style={{ color:"#e2e8f0" }}>{r.missing_skills.join(", ")}</span>
                </div>
              )}
            </div>
          ))}
