    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                let stats = statsRes.ok ? await statsRes.json() : null;
                let proposals = proposalsRes.ok ? await proposalsRes.json() : [];

                setActiveContracts(Array.isArray(active) ? active : []);
                setCompletedContracts(Array.isArray(completed) ? completed : []);
                setStats(stats && Object.keys(stats).length > 0 ? stats : null);
