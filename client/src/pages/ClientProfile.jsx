...
import { Building2, Globe, MapPin, Linkedin, Eye, EyeOff } from "lucide-react";
import { createClientProfile, getClientProfile } from "../services/api";
...
export default function ClientProfile() {
  ...
  // Pre-fill form with existing profile data on load
  useEffect(() => {
  ...
  const onSubmit = async (data) => {
    const storedUser = localStorage.getItem("user");
    if (!storedUser) {
      setErrorMsg("You must be logged in to save your company profile.");
      return;
    }
    const user = JSON.parse(storedUser);
    ...
    setSaving(true);
    try {
      await createClientProfile(user.id, { ...data, is_public: isPublic });
      setSuccessMsg("Company profile saved!");
      setErrorMsg("");
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-8">
          <p className="text-sm text-gray-500 mb-1">
            Dashboard / Client / <span className="text-orange-600">Profile</span>
          </p>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div>
                ...
                <input
                  ...
                  className="w-full p-2.5 bg-white border border-gray-200 rounded-lg focus:ring-2 focus:ring-orange-500 transition"
          ...
          {/* Footer */}
          {successMsg && (
            <div className="mx-8 mb-0 mt-0 p-3 bg-green-50 text-green-700 text-sm rounded-lg border border-green-200">
              {successMsg}
            </div>
          )}
          {errorMsg && (
            <div className="mx-8 mb-0 mt-0 p-3 bg-red-50 text-red-700 text-sm rounded-lg border border-red-200">
              {errorMsg}
            </div>
          )}
          <div className="p-6 bg-gray-50 border-t border-gray-200 flex justify-end gap-3">
            <button
              ...
              className="px-6 py-2.5 border border-gray-300 text-gray-700 font-medium rounded-lg hover:bg-white transition bg-white shadow-sm"
            >
              Cancel
            </button>
            <button
              type="submit"
              disabled={saving}
              className={`px-6 py-2.5 bg-orange-500 text-white font-medium rounded-lg hover:bg-orange-600 shadow-sm shadow-orange-200 transition ${saving ? "opacity-70 cursor-not-allowed" : ""}`}
            >
