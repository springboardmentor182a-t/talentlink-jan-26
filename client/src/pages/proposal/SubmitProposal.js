...
export default function SubmitProposal() {
  ...
  const inp = { width: "100%", padding: "12px 14px", border: "1.5px solid #e5e7eb", borderRadius: 8, fontSize: 14, backgroundColor: "#f9fafb", outline: "none", boxSizing: "border-box", fontFamily: "inherit" };
    ...
    <div style={{ minHeight: "100vh", backgroundColor: "#f0f2f5", fontFamily: "'Segoe UI',sans-serif" }}>
      ...
      <div style={{ backgroundColor: "#fff", padding: "16px 32px", display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "1px solid #e5e7eb" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <div style={{ textAlign: "right" }}>
            <div style={{ fontWeight: 600, fontSize: 14, color: "#111827" }}>{user.name || user.full_name}</div>
            <div style={{ fontSize: 12, color: "#6b7280", textTransform: "capitalize" }}>{role}</div>
          </div>
          <button onClick={() => { logout(); navigate("/"); }}
            style={{ padding: "8px 16px", border: "1px solid #e5e7eb", borderRadius: 8, cursor: "pointer", fontSize: 13, color: "#374151", background: "white", fontFamily: "inherit" }}>
            Logout
          </button>
        </div>
      </div>
      ...
      <div style={{ padding: 32, maxWidth: 760, margin: "0 auto" }}>
        <button onClick={() => navigate("/freelancer/dashboard")}
          style={{ background: "none", border: "none", fontSize: 14, color: "#6b7280", cursor: "pointer", padding: 0, marginBottom: 20, fontFamily: "inherit" }}>
          ← Back to Dashboard
        </button>
        ...
        <div style={{ backgroundColor: "#fff", borderRadius: 12, padding: 32, boxShadow: "0 1px 4px rgba(0,0,0,0.07)" }}>
          ...
          <button onClick={handleSubmit} disabled={loading}
            style={{ width: "100%", padding: 14, background: loading ? undefined : "linear-gradient(135deg,#7c3aed,#a855f7)", backgroundColor: loading ? "#9ca3af" : undefined, color: "white", border: "none", borderRadius: 8, cursor: loading ? "not-allowed" : "pointer", fontWeight: 700, fontSize: 15, marginTop: 16, fontFamily: "inherit" }}>
