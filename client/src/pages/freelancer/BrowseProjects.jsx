      ...
      {selected && (
        <div onClick={closeModal} style={{ position:"fixed", inset:0, backgroundColor:"rgba(15,23,42,0.6)", zIndex:1000, display:"flex", alignItems:"center", justifyContent:"center", padding:24, backdropFilter:"blur(4px)" }}>
          <div onClick={e => e.stopPropagation()}
            style={{ backgroundColor:"var(--card)", borderRadius:20, padding:32, maxWidth:660, width:"100%", maxHeight:"90vh", overflowY:"auto", boxShadow:"0 20px 60px rgba(0,0,0,0.3)", border:"1px solid var(--border)" }}>
            ...
            {/* Tabs */}
            <div style={{ display:"flex", gap:0, marginBottom:24, backgroundColor:"var(--input-background)", borderRadius:10, padding:4 }}>
              <button onClick={() => setShowClientProfile(false)}
                style={{ flex:1, padding:"8px 0", borderRadius:8, border:"none", cursor:"pointer", fontWeight:600, fontSize:13,
                  background: !showClientProfile ? "var(--card)" : "transparent",
                  color: !showClientProfile ? "#7c3aed" : "var(--text-faint)",
            ...
            {!showClientProfile && (
              <>
                ...
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:16, marginBottom:24, backgroundColor:"var(--page-bg)", borderRadius:12, padding:16, border:"1px solid var(--border)" }}>
                  <div>
                    <div style={statLabel2}>Budget</div>
                    <div style={{ fontSize:20, fontWeight:800, color:"var(--text-primary)" }}>${Number(selected.budget).toLocaleString()}</div>
                  </div>
                  <div>
                    <div style={statLabel2}>Deadline</div>
                    <div style={{ fontSize:16, fontWeight:700, color:"var(--text-primary)" }}>{selected.deadline || "—"}</div>
                  </div>
                  <div>
                    <div style={statLabel2}>Posted</div>
            ...
            {showClientProfile && (
              <div>
                {clientLoading && <div style={{ textAlign:"center", padding:"40px 0", color:"var(--text-muted)" }}>⏳ Loading client profile...</div>}
                {!clientLoading && !clientProfile && <div style={{ textAlign:"center", padding:"40px 0", color:"var(--text-faint)" }}>⚠️ Client profile not available.</div>}
                  ...
                  <>
                    ...
                    {clientProfile.bio && (
                      <div style={{ marginBottom:16 }}>
                        ...
                        <div style={{ fontSize:14, color:"var(--text-secondary)", lineHeight:1.8, padding:"12px 14px", backgroundColor:"var(--page-bg)", borderRadius:10, border:"1px solid var(--border)" }}>{clientProfile.bio}</div>
                      </div>
                    )}
                      ...
                      <div style={{ marginBottom:16 }}>
                        ...
                        <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                          {clientProfile.reviews.slice(0,3).map((r, i) => (
                            <div key={i} style={{ backgroundColor:"var(--page-bg)", borderRadius:10, padding:"12px 14px", border:"1px solid var(--border)" }}>
                              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:6 }}>
                                <div style={{ fontWeight:600, fontSize:13, color:"var(--text-primary)" }}>{r.reviewer_name}</div>
                                <div style={{ display:"flex", gap:2 }}>
                                  {[1,2,3,4,5].map(star => (
                                    <span key={star} style={{ fontSize:14, color: star <= r.rating ? "#f59e0b" : "var(--border)" }}>★</span>
                                  ))}
                                </div>
                              </div>
                    ...
                    {clientProfile.reviews?.length === 0 && (
                      <div style={{ textAlign:"center", padding:"16px 0", color:"var(--text-faint)", fontSize:13 }}>No reviews yet for this client.</div>
                    )}
...
function ClientField({ icon, label, value }) {
  return (
    <div>
      <div style={{ fontSize:11, fontWeight:700, color:"var(--text-faint)", textTransform:"uppercase", letterSpacing:"0.5px", marginBottom:5 }}>{icon} {label}</div>
      <div style={{ fontSize:14, color: value ? "var(--text-primary)" : "var(--text-faint)", padding:"8px 12px", backgroundColor:"var(--page-bg)", borderRadius:8, border:"1px solid var(--border)" }}>{value || "—"}</div>
    </div>
  );
}

const modalLabel = { fontSize:11, fontWeight:700, color:"var(--text-faint)", textTransform:"uppercase", letterSpacing:"0.5px", display:"block", marginBottom:8 };
const statLabel2 = { fontSize:11, color:"var(--text-faint)", textTransform:"uppercase", letterSpacing:"0.5px", marginBottom:4 };
