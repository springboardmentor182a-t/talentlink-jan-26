  ...
  return (
    <div style={{ fontFamily:"'Segoe UI',sans-serif", backgroundColor:"#0f172a", minHeight:"100vh" }}>
      ...
      <div style={{ padding:32 }}>
        ...
        {/* Filters */}
        <div style={{ backgroundColor:"#1e293b", borderRadius:16, padding:"20px 24px", marginBottom:24, boxShadow:"0 1px 3px rgba(0,0,0,0.06)", border:"1px solid #334155" }}>
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:16 }}>
            <span style={{ fontSize:15, fontWeight:700, color:"#f8fafc" }}>🔽 Filters</span>
        ...
        {!loading && filtered.length === 0 && (
          <div style={{ backgroundColor:"#1e293b", borderRadius:16, padding:"60px 32px", textAlign:"center", boxShadow:"0 1px 3px rgba(0,0,0,0.06)", border:"1px solid #334155" }}>
            ...
            <h3 style={{ fontSize:18, fontWeight:700, color:"#f8fafc", marginBottom:8 }}>No projects found</h3>
            <p style={{ color:"#94a3b8", fontSize:14, marginBottom:16 }}>Try adjusting your filters to find more projects.</p>
            <button onClick={clearFilters}
          ...
          <div key={p.id} style={{ backgroundColor:"#1e293b", borderRadius:16, padding:24, marginBottom:16, boxShadow:"0 1px 3px rgba(0,0,0,0.06)", border:"1px solid #334155" }}
            ...
            <div style={{ display:"flex", gap:24, paddingTop:16, borderTop:"1px solid #f1f5f9", marginBottom:16 }}>
              <div>
                ...
                <strong style={{ color:"#f8fafc", fontSize:15 }}>${Number(p.budget).toLocaleString()}</strong>
              </div>
              <div>
            ...
            <div style={{ display:"flex", gap:12, flexWrap:"wrap" }}>
              ...
              <button onClick={() => setSelected(p)}
                style={{ padding:"10px 24px", backgroundColor:"#1e293b", color:"#e2e8f0", border:"1.5px solid #e2e8f0", borderRadius:8, cursor:"pointer", fontWeight:600, fontSize:14 }}>
                View Details
              </button>
              ...
              <button onClick={() => checkMatchScore(p.id)}
                ...
                style={{ padding:"10px 24px", backgroundColor:"#1e293b", color:"#7c3aed", border:"1.5px solid #7c3aed", borderRadius:8, cursor:"pointer", fontWeight:600, fontSize:14, opacity: matchLoading[p.id] ? 0.7 : 1 }}>
              ...
              <div style={{ marginTop:16, padding:16, backgroundColor:"#f5f3ff", borderRadius:12, border:"1px solid #ddd6fe" }}>
                <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:10 }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize:12, color:"#94a3b8" }}>{matchScores[p.id].summary}</div>
                  </div>
                </div>
                {matchScores[p.id].matching_skills?.length > 0 && (
                  <div style={{ marginBottom:6 }}>
                    <span style={{ fontSize:12, fontWeight:600, color:"#16a34a" }}>✅ Matching: </span>
                    <span style={{ fontSize:12, color:"#e2e8f0" }}>{matchScores[p.id].matching_skills.join(", ")}</span>
                  </div>
                )}
                {matchScores[p.id].missing_skills?.length > 0 && (
                  <div>
                    <span style={{ fontSize:12, fontWeight:600, color:"#dc2626" }}>❌ Missing: </span>
                    <span style={{ fontSize:12, color:"#e2e8f0" }}>{matchScores[p.id].missing_skills.join(", ")}</span>
                  </div>
                )}
              </div>
            )}
      ...
      {selected && (
        <div onClick={() => setSelected(null)}
          ...
          <div onClick={e => e.stopPropagation()}
            style={{ backgroundColor:"#1e293b", borderRadius:20, padding:32, maxWidth:600, width:"100%", maxHeight:"80vh", overflowY:"auto", boxShadow:"0 20px 60px rgba(0,0,0,0.3)", border:"1px solid #334155" }}>

            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:20 }}>
              <div>
                <h2 style={{ fontSize:22, fontWeight:800, color:"#f8fafc", margin:"0 0 8px", letterSpacing:"-0.5px" }}>{selected.title}</h2>
            ...
            <div style={{ marginBottom:20 }}>
              <label style={{ fontSize:11, fontWeight:700, color:"#94a3b8", textTransform:"uppercase", letterSpacing:"0.5px", display:"block", marginBottom:8 }}>Description</label>
              <p style={{ fontSize:14, color:"#e2e8f0", lineHeight:1.8, margin:0, backgroundColor:"#0f172a", padding:"14px 16px", borderRadius:10, border:"1px solid #334155" }}>{selected.description}</p>
            </div>

            {selected.skills && (
              <div style={{ marginBottom:20 }}>
                <label style={{ fontSize:11, fontWeight:700, color:"#94a3b8", textTransform:"uppercase", letterSpacing:"0.5px", display:"block", marginBottom:8 }}>Required Skills</label>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:16, marginBottom:24, backgroundColor:"#0f172a", borderRadius:12, padding:16, border:"1px solid #334155" }}>
              <div>
                <div style={{ fontSize:11, color:"#94a3b8", textTransform:"uppercase", letterSpacing:"0.5px", marginBottom:4 }}>Budget</div>
                <div style={{ fontSize:20, fontWeight:800, color:"#f8fafc" }}>${Number(selected.budget).toLocaleString()}</div>
              </div>
              <div>
                <div style={{ fontSize:11, color:"#94a3b8", textTransform:"uppercase", letterSpacing:"0.5px", marginBottom:4 }}>Deadline</div>
                <div style={{ fontSize:16, fontWeight:700, color:"#f8fafc" }}>{selected.deadline || "—"}</div>
              </div>
              <div>
                <div style={{ fontSize:11, color:"#94a3b8", textTransform:"uppercase", letterSpacing:"0.5px", marginBottom:4 }}>Posted</div>
                <div style={{ fontSize:14, fontWeight:600, color:"#f8fafc" }}>{new Date(selected.created_at).toLocaleDateString()}</div>
              </div>
            </div>
            ...
            <div style={{ display:"flex", gap:12 }}>
              ...
              <button onClick={() => setSelected(null)}
                style={{ padding:"13px 24px", backgroundColor:"#1e293b", color:"#94a3b8", border:"1.5px solid #e2e8f0", borderRadius:10, cursor:"pointer", fontWeight:600, fontSize:14 }}>
                Close
              </button>
            </div>
          </div>
...
const lbl = { display:"block", fontSize:13, fontWeight:500, color:"#e2e8f0", marginBottom:6 };
