...
def read_dashboard(
    ...
    client_id = current_user.id if current_user else default_client.id

    return service.get_dashboard_data(db, client_id)


@router.get("/received-proposals")
def read_received_proposals(
    db: Session = Depends(SessionLocal),
    current_user: User = Depends(get_current_user)
):
    """
